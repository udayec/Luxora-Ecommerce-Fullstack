...
export const increaseCartQuantity = 
    ...
    (dispatch, getState) => {
        ...
        if (isQuantityExist) {
            ...
            setCurrentQuantity(newQuantity);  //update the UI

            dispatch({
                type: "ADD_CART",   //updated in the redux store using this
                payload: {...data, quantity: newQuantity + 1 },
            });
            localStorage.setItem("cartItems", JSON.stringify(getState().carts.cart));  //update in the localStorage
        } else {
...
export const registerNewUser 
    = (sendData, toast, reset, navigate, setLoader) => async (dispatch) => {
        try {
            ...
            reset();
            toast.success(data?.message || "User Registered Successfully");   //this data is coming from the backend
            navigate("/login");
        } catch (error) {
            console.log(error);
...
export const addUpdateUserAddress =
    ...
    /*const { user } = getState().auth;   for jwtToken
...
export const getAddressById = (addressId) => async (dispatch) => {
  try {
    dispatch({ type: "FETCH_ADDRESS_REQUEST", addressId });

    const { data } = await api.get(`/addresses/${addressId}`);

    dispatch({ type: "FETCH_ADDRESS_SUCCESS", payload: { addressId, data } });
  } catch (error) {
    dispatch({
